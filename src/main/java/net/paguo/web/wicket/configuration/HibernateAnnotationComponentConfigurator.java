package net.paguo.web.wicket.configuration;
...
public class HibernateAnnotationComponentConfigurator extends AbstractBehavior
    ...
    private final static Map<Class<? extends Annotation>, HibernateAnnotationConfig> configs;

    static {
        configs = new HashMap<Class<? extends Annotation>, HibernateAnnotationConfig>() {
            {
                put(NotNull.class, new HibernateAnnotationConfig() {
                    public void onAnnotatedComponent(Annotation annotation,
                                                     FormComponent component) {
                        component.setRequired(true);
                        component.add(new AttributeModifier("style", true, new Model("background-color:#FAEBD7;")));
                    }
                });
                put(Length.class, new HibernateAnnotationConfig() {
                    public void onAnnotatedComponent(Annotation annotation,
                        ...
                        component.add(StringValidator.lengthBetween(min, max));
                    }
                });
                put(Email.class, new HibernateAnnotationConfig() {
    ...
    //phone: ""\\b(\\+?0?\\d+)?.?(\\d{3}).?(\\d{3}-?\\d{2}-?\\d{2})\\b"
    // "\\b(\\+?0?\\d+)?.?(\\d{3}).?(\\d{3}-?\\d{2}-?\\d{2})\\b"
